<nav x-data="{ open: false }" class="w-full px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between h-16">
        
        <div class="flex items-center">
            <button @click="sidebarOpen = !sidebarOpen" class="inline-flex items-center justify-center p-2 rounded-md text-emerald-600 hover:text-emerald-800 hover:bg-emerald-50 focus:outline-none focus:bg-emerald-50 focus:text-emerald-800 transition duration-150 ease-in-out lg:hidden">
        ...
        <div class="flex items-center">
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-semibold rounded-md text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:text-emerald-600 dark:hover:text-emerald-400 focus:outline-none transition ease-in-out duration-150">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-white border-t border-gray-100">
        <div class="pt-4 pb-1">
            ...
            <div class="mt-3 space-y-1">
                <x-responsive-nav-link :href="route('profile.edit')" class="text-emerald-700 focus:text-emerald-800 focus:bg-emerald-50">
                    {{ __('Profile') }}
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <x-responsive-nav-link :href="route('logout')"
                            onclick="event.preventDefault(); this.closest('form').submit();"
                            class="text-red-600 focus:text-red-800 focus:bg-red-50">
                        {{ __('Log Out') }}
                    </x-responsive-nav-link>
                </form>
            </div>
